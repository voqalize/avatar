...
"""Static server for the workshop pages.
    ...
    python3 authoring/serve.py [port]        # default 8777
    open http://localhost:8777/authoring/

It serves the **repository root**, not this directory, and does so from
wherever you run it. The pages import `../src/*.js` by relative path — that is
the point of them, since `src/` has no build step and what you screenshot is
what ships — so a server rooted at `authoring/` would 404 the entire library.
...
print(f'serving {ROOT} on http://localhost:{port}/authoring/')
