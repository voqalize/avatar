...
**These commands are the whole protocol.**
...
Spellings older than the redesign are accepted and translated at the client's
parse boundary — `cmd: "claim"` for `state`, `cmd: "sequence"` for `action`,
`STRAINING` for `CANT_HEAR`. None appears above that boundary, and they come out
when the last server that sends them has shipped
(`parseAvatarCommand` in `packages/avatar/client/types.ts` is the one place that
knows them).

## State

The behavior vocabulary is `BEHAVIOR_STATE_IDS` in
`packages/avatar/src/behavior.js`; the names the table above allows are the
subset a server may send. Every other one is a fact the browser already holds
from Pipecat's own events and its own clock, and a server spelling of a fact
would be a second, lower-authority copy of it. What is left here is exactly what
nothing in the browser can see: the agent is waiting on a model, running a tool,
or has stopped waiting for a reply.
...
**`ACKNOWLEDGE` is one word on purpose.** Which shape a face makes of it is a
rendering decision the server is not holding the drawing for; the mapping lives
in `packages/avatar/src/behavior.js`. The server's job is to know that an
...
reads ([design-avatar-interface.md](design-avatar-interface.md)).
...
meaning. A renderer's catalogue is its own and this document does
not enumerate it — that is the point of the id being open.
...
`ctx` is Pipecat's opaque TTS `context_id`, and it carries one obligation on the
server: **send a context's first patch only once that context's first audio
sample has passed you.** The text-predicted track exists sooner, but the client
binds contexts to speaking events FIFO, so a context interrupted before any of
its audio plays must never enter that queue — or every later reply plays the
cues of the one before it. How the client binds and anchors them is
[pipecat-lifecycle-protocol.md](pipecat-lifecycle-protocol.md)'s.
...
Only one is in flight at a time — a later one replaces the earlier — so these
names carry no ordering on this wire. Which condition wins when several
