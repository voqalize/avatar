 ...
 * advertised a `TYPING` state for weeks after `packages/avatar/src/avatar.js`
 * renamed it `WORKING`, and listed `WAVE`/`THUMBS_UP` clips that had become
...
/** `packages/avatar/test/` → the repo root, three up. */
...
const MANIFEST_FILES = [
  "packages/avatar-py/pyproject.toml",
...
const GENERATED = new Set([
  // Staged by packages/avatar-py/scripts/stage_native.py immediately before a
  // wheel build, and named in pyproject.toml precisely BECAUSE it is
  // gitignored — hatchling has to be told the exclusion is deliberate.
  "packages/avatar-py/src/voqalize_avatar/_native",
  // tsc output. Present after `pnpm run build`, absent on a fresh clone, and
  // the docs name it because it is what the tarball's entrypoint is.
  "packages/avatar/dist",
]);

/** A GENERATED entry stands for the tree under it, not just the directory. */
const isGenerated = (path: string) =>
  [...GENERATED].some((g) => path === g || path.startsWith(g + "/"));
...
describe("the docs", () => {
   ...
   * months. `packages/avatar-py/pyproject.toml` shipped `docs/contract-protocol.md` as PyPI
  ...
  it("has paths to check against", () => {
    ...
    expect(found).toContain("packages/avatar/src/avatar.js");
  });
});
