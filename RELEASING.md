...
package: the runtime wrapper is ordinary stdlib Python that takes the binary's
location as an argument, and the 56 MB acoustic model is regenerated by
`build.sh --res-only` rather than shipped. Consumers ship it the way they
already ship large artifacts — a `COPY` into an image, or an existing build
cache — and pass the directory to `build_viseme_engine`. See
