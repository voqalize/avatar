 ...
 *
 *   node apps/authoring/tools/baseline.mjs [outDir]        # default .review/baseline-<sha>/
 *
...
try {
  ...
  for (const name of names) {
    jobs.push([`${name}-contact-sheet.png`, `apps/authoring/contact-sheet.html?face=${name}`]);
    jobs.push([`${name}-torso-check.png`, `apps/authoring/torso-check.html?face=${name}`]);
    for (const clip of CLIPS) {
      jobs.push([`${name}-clip-${clip}.png`, `apps/authoring/clip-strip.html?face=${name}&clip=${clip}`]);
    }
  }
