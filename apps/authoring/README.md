...
it ships: the published library is `packages/avatar/src/` and `packages/avatar/client/`, the demo call is
`apps/server/`, and the IDE for the published interface is `apps/studio/`. This is the
...
has a row in `packages/avatar/src/faces.js`.
...
`pnpm test` (`packages/avatar/src/conformance.js` holds the assertions). `rig-check.html`'s
