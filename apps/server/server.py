...
- **The page.** Served from this repo's working tree — the whole of
  `packages/avatar/`, plus one pre-bundled copy of pipecat's browser packages,
  all with `Cache-Control: no-store`. Nothing of ours is bundled: the widget is
...
# Where each URL prefix comes from. `/pkg` is the published package directory,
# mounted whole and served from the working tree — so `/pkg/dist/index.js`
# importing `../src/avatar.js` resolves to `/pkg/src/avatar.js` with no
# arrangement here at all. That import is an ordinary within-package sibling
# now; this mount used to have to mirror the repo's directory depth so the
# browser could follow a `../../src/` out of one top-level tree and into
# another, and that is the coupling the merge removed.
#
...
MOUNTS = {
    "/pkg": REPO / "packages" / "avatar",
    "/vendor": HERE / "vendor",
}

MISSING = {
    "/pkg": "run `pnpm install && pnpm run build`",
    "/vendor": "run `pnpm run server:vendor`",
}

# `/pkg` is a source directory and is therefore always there, so its mount alone
# proves nothing — the half that is built has to be checked by name or the page
# fails later, in the browser, as a 404 on a module.
REQUIRED = {"/pkg": "dist/index.js"}
...
def build_app(tts_name: str) -> FastAPI:
    ...
    for url, path in MOUNTS.items():
        needed = path / REQUIRED[url] if url in REQUIRED else path
        if not path.is_dir() or not needed.exists():
            raise SystemExit(f"missing {needed} — {MISSING[url]} (in {REPO})")
