...
`packages/avatar/src/avatar.js`, not `packages/avatar/client/AvatarClient.ts`, not `@voqalize/avatar/internal`
...
- **Nothing here asks the avatar anything.** `createAvatar` returns no resolved
  ...
  reads — `apps/studio/src/presence.ts`, written from
...
is an HTTP request to `apps/server/`, which then sends the message — a page that could
...
change ([packages/avatar-py/README.md](../../packages/avatar-py/README.md)).

The **voice** sits in the build panel but in a band of its own, because it is
not a `createAvatar` argument — it is which voice `apps/server/` speaks in, and it
reaches the face only as audio ([apps/server/README.md § Two voices](../server/README.md)).
...
- **Not the 30-second look.** That is `apps/server/index.html`
  ([apps/server/README.md](../server/README.md)) — same call, same palette, no build
  step, no options.
- **Not a rig workbench.** Channel extremes, viseme close-ups, clip strips and
  ...
  `apps/authoring/` ([apps/authoring/tools/README.md](../authoring/tools/README.md)). Studio validates the
