...
| 2.5-D characters | `tara`, `tushar`, `tanya`, `tess`, `tanvi` — one `createAvatar` each over one shared rig; `three` is an *optional* peer, so a drawing never pays for a 3-D engine | `packages/avatar/client/three/`, `packages/avatar/assets/*.glb` | [characters.md](docs/characters.md) |
